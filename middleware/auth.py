...
async def require_internal_secret(
    ...
    if x_api_key and settings.LEGACY_API_KEY and x_api_key == settings.LEGACY_API_KEY:
...
def _matches_secret(provided: str, settings) -> bool:
    if settings.INTERNAL_SECRET and provided == settings.INTERNAL_SECRET:
        return True
    if settings.INTERNAL_SECRET_PREVIOUS and provided == settings.INTERNAL_SECRET_PREVIOUS:
