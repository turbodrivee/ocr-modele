...
def test_permis_full():
    raw = (
        ...
        "1. 23/ 238943 3. KARKOUB 4. Mohamed 5. 12-05-1990 10. 30-12-2030 B"
    )
...
def test_permis_multiple_categories_sorted_unique():
    raw = "PERMIS 12345678 B C C4 A 5. 01-01-1990"
    out = parse_fields(raw, "permis")
...
def test_permis_three_distinct_dates_eu_markers():
    # EU markers 5./4a./4b. — all three dates labeled distinctly.
    raw = (
        "PERMIS 11223344 3. BENALI 4. Sami "
        "5. 14-02-1988 4a. 10-06-2018 4b. 09-06-2028 B"
    )
    out = parse_fields(raw, "permis")
    assert out["date_naissance"] == "1988-02-14"
    assert out["date_delivrance"] == "2018-06-10"
    assert out["date_expiration"] == "2028-06-09"
    assert out["warnings"] == []
...
def test_permis_french_labels():
    raw = (
        ...
        "Né le 12/05/1990 Délivré le 03/04/2022 Valable jusqu'au 02/04/2032 B"
    )
    out = parse_fields(raw, "permis")
    assert out["date_naissance"] == "1990-05-12"
    assert out["date_delivrance"] == "2022-04-03"
    assert out["date_expiration"] == "2032-04-02"
...
def test_permis_chronological_fallback_three_dates():
    # No labels: three bare dates, parser must order them chronologically.
    raw = "PERMIS 33445566 B 05/06/1985 12/03/2020 30/12/2030"
    out = parse_fields(raw, "permis")
...
def test_permis_only_two_dates_warns():
    # No delivrance date present at all — fallback fills naissance+expiration only.
    raw = "PERMIS 44556677 B 01/01/1985 30/12/2030"
    out = parse_fields(raw, "permis")
    assert out["date_naissance"] == "1985-01-01"
    assert out["date_expiration"] == "2030-12-30"
    assert out["date_delivrance"] is None
    assert "date_delivrance_unidentified" in out["warnings"]


def test_permis_expired_license_warning():
    # Expiration labeled but in the past — keep the value, raise a warning.
    raw = "PERMIS 55667788 5. 12-05-1990 4a. 10-06-2010 4b. 09-06-2020 B"
    out = parse_fields(raw, "permis")
...
def test_permis_iso_format_consistency():
    ...
    for key in ("date_naissance", "date_delivrance", "date_expiration"):
